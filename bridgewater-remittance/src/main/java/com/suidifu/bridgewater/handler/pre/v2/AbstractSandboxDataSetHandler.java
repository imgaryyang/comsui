    ...
    public PedestrianBankCode existBank(String bankName, int headQuarter) {
    	...
    	if (headQuarter==1) {
        	...
        	if (CollectionUtils.isEmpty(bankList)) {
        	 ...
        	 list=bankList.stream().filter(bank->bank.getBankFullName().contains(bankName)).collect(Collectors.toList());
...
//        LOGGER.info("bankName====="+subBankName+"\tPedestrianBankCode size"+bankList.size());
        ...
        if(StringUtils.isEmpty(subBankName)){
            remittanceDetail.put("bankName", ZhonghangResponseMapSpec.HEAD_OFFICE_ENGLISH_ABBR_CN_MAP.get(bankAliasName));
        }
