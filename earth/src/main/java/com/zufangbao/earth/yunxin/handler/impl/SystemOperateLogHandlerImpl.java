...
public class SystemOperateLogHandlerImpl implements SystemOperateLogHandler {
	...
	private String generateRecordContent(String objectUuid,
		...
		case EXPORTREPAYEMNTPLAN:
			...
			break;
		case INTERFACEPAYMENTEXPORTDAILYLIST:
			recordContent = "导出" + "接口线上支付单每日清单"+  objectUuid;
			break;
		case INTERFACEEXPORTACCOUNTCHECKING:
			recordContent = "导出" + "接口线上支付单每日对账单" +objectUuid;
			break;
		default:
	...
	private String getOperateName(SystemOperateLog systemOperateLog) {
		return -1L == systemOperateLog.getUserId() ? "SYSTEM"
				: principalService.load(Principal.class, systemOperateLog.getUserId()).getName();
	}
	...
	private String castUpdateRecordContent(LogFunctionType logfunctionType,
		...
		for (UpdateContentDetail updateDetail : updateDetails) {
			//判断是否添加,信息未更改,则不添加
			if(!updateDetail.getOldValue().equals(updateDetail.getNewValue())){
				stringBuffer.append(
						"," + LogMapRecordContentSpec.recordContentHeadNameMatchRecordContent.get(
								LogMapRecordContentSpec.logFunctionTypeMatchRecordContentHeadName.get(logfunctionType)).get(updateDetail.getFiledName())
								+ "由【" + updateDetail.getOldValue() + "】变更为【"
	...
	@Override
	public void operateLog(Principal principal, String ip, LogFunctionType functionType,
							LogOperateType logOperateType, Object object, String keyContent, String recordContent ) throws Exception {
		SystemOperateLog log = recordLogCore.insertNormalRecordLog(principal.getId(),ip, functionType, logOperateType, object);
		log.setKeyContent(keyContent);
