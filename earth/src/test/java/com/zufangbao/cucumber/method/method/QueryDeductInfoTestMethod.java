...
@Component("queryDeductInfoTestMethod")
public class QueryDeductInfoTestMethod extends BaseApiTestPost{
	...
	public String query_i_RepaymentPlan(String uniqueId,int i){
		...
		try {
			String sr = PostTestUtil.sendPost(QUERY_URL, requestParams1, getIdentityInfoMap(requestParams1));
...
//		requestParams.put("payerName", "xiewenqian3"); 
//		requestParams.put("payAcNo", "6214855712106500"); 
...
//				+ "'penaltyFee':1.00,'latePenalty':1.00,'lateFee':1.00,'lateOtherCost':1.00,'totalOverdueFee':4.00}}]");
		...
		try {
			String sr = PostTestUtil.sendPost(DEDUCT_URL, requestParams, getIdentityInfoMap(requestParams));
			System.out.println(sr);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();	
	...
	public void deductInfoList1(String uniqueId,String deductId) {
		...
		requestParams.put("uniqueId", uniqueId);
		try {
			String sr = PostTestUtil.sendPost(QUERY_URL, requestParams, getIdentityInfoMap(requestParams));
			System.out.println(sr);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
