...
@Transactional
public class DataStatisticsCacheHandlerTest extends AbstractTransactionalJUnit4SpringContextTests{
	@Autowired
	...
	public void testGetData_when_time_out(){
		...
		AssetSet assetSet = new AssetSet(contract, 3, DateUtils.asDay(new Date()),new BigDecimal("1000"), new BigDecimal("10"),BigDecimal.ZERO,"merId+repayScheduleNo","repayScheduleNo");
	...
	public void testGetData_when_evictCache(){
		...
		//插入一条新的逾期处理中的asset
		Contract contract = contractService.load(Contract.class, 1L);
		AssetSet assetSet = new AssetSet(contract, 3, new Date(),new BigDecimal("1000"), new BigDecimal("10"),BigDecimal.ZERO,"merId+repayScheduleNo","repayScheduleNo");
		assetSet.setOverdueStatus(AuditOverdueStatus.OVERDUE);
		repaymentPlanService.save(assetSet);
