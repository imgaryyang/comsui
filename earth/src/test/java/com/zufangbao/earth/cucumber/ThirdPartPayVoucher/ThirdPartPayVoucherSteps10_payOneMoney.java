...
public class ThirdPartPayVoucherSteps10_payOneMoney extends commandUrl {
    @Autowired
    ...
    String transactionRequestNo="70202808258158592";//交易请求号
    String productCode="G31700";
    String totalamount="30";
    String deductamount="";
    String partamount="20";//剩余两期金额
    String amount="10";

    @Given("^有一个放款计划，对应有三期还款计划ten$")
    public void 有一个放款计划_对应有三期还款计划ten() throws Throwable {
    ...
    @Given("^有一期已经部分还钱,还款人去第三方机构还款，并且产生一个第三方凭证ten$")
    public void 有一期已经部分还钱_还款人去第三方机构还款_并且产生一个第三方凭证ten() throws Throwable {
        // 查询还款计划编号
        String RepaymentPlanNo1=baseTestMethod.queryFirstRepaymentPlan(uniqueId,0);
        System.out.println("查询部分还钱的还款编号:"+RepaymentPlanNo1);
        //对还款计划进行部分扣款
        System.out.println("对还款计划进行部分扣款:");
        baseTestMethod.deductRepaymentPlan(deductUrl,RepaymentPlanNo1,uniqueId,productCode,amount,"0");
        //申请提前还款
        System.out.println("申请提前还款:");
        baseTestMethod.applyPrepaymentPlan(uniqueId,partamount,partamount,DateUtils.format(DateUtils.addDays(new Date(),0)));
        // 开始制造第三方凭证
        System.out.println("开始制造第三方凭证:");
        //有申请提前还款接口就使用MakeVOucher4，没有则使用MakeVOucher3
//        baseTestMethod.MakeVOucher4(NewOuterInternat_command,transactionRequestNo,uniqueId,productCode);
    ...
    public void 获取第三方交易机构的交易记录_第三方凭证和交易记录进行校验ten() throws Throwable {
        // 插入第三方交易记录的流水记录
        ...
        thirdPartyTransactionRecord.setTransactionAmount(new BigDecimal("1300"));//amount
    ...
    public void 校验应该自动进行_检验是否能执行ten() throws Throwable {
        // Write code here that turns the phrase above into concrete actions
    }
