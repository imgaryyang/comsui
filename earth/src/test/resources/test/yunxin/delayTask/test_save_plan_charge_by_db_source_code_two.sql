...
Map<String, Object> data = (Map<String, Object>) processedResult.getData();
            List<String> repaymentPlanNoTarList = new ArrayList<String>();
            List<JSONObject> repaymentPlanList = (List<JSONObject>)data.getOrDefault("repaymentPlanList", Collections.emptyList());
            for (JSONObject jsonObject :repaymentPlanList) {
                String repaymentNumber = jsonObject.getString("repaymentNumber");
                repaymentPlanNoTarList.add(repaymentNumber);
            }
            log.info("repaymentPlanNoTarList:" + com.zufangbao.sun.utils.JsonUtils.toJSONString(repaymentPlanNoTarList));
...
log.info("sandboxDataSetTar:" + com.zufangbao.sun.utils.JsonUtils.toJSONString(sandboxDataSetTar));
            String reason = (String) sandboxDataSetTar.getExtraData().getOrDefault("reasonCode", "0");
...
log.info("reasonCode" + reasonCode);
            ...
            if (RepaymentPlanModifyReason.REASON_7.getOrdinal() == reasonCode) {
                ...
                changeType = "6";
            } else if (RepaymentPlanModifyReason.REASON_11.getOrdinal() == reasonCode) {
                changeType = "7";
            } else if (RepaymentPlanModifyReason.REASON_12.getOrdinal() == reasonCode) {
                changeType = "8";
            } else if (RepaymentPlanModifyReason.REASON_13.getOrdinal() == reasonCode) {
                changeType = "9";
            }
            if (changeType == null) {
                //return false;
...
log.info("changeType" + changeType);
            ...
            for (PaymentPlanSnapshot paymentPlanSnapshot : paymentPlanSnapshotList) {
               ...
               if("HA0100".equals(sandboxDataSetTar.getFinancialContractSnapshot().getContractNo())){
                    map.put("tradeNo", UUID.randomUUID().toString().replaceAll("-", ""));
                }else{
                    map.put("tradeNo", "BQ"+uniqueId);
                }
...
//所有第0期，应收表中的应收金额不用做减法
                ...
                if (0 == paymentPlanSnapshot.getCurrentPeriod()) {
...
log.info("taskJSONString:" + taskJSONString);

                delayProcessingTaskHandler.save_to_db_cache(task);
log.info("delayProcessingTaskHandler.save_to_db_cache end");
            }
            return true;
        } catch (Exception e) {
            e.printStackTrace();
