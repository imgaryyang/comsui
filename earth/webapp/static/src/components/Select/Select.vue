<style lang="sass">
  @import '~assets/themes/default/select.css';
  @import '~assets/themes/default/option.css';
  ...
  export default {
    ...
    computed: {
      selectedText: function() {
        var r = this.selected.map(item => item.currentLabel);
        return r.join('，');
      },
    ...
    watch: {
      selectedLabel(v){
        if(!this.remote && this.filterable && this.allowCreate){
          this.query = v
        }
      },
      placeholder(val) {
        ...
        this.$nextTick(() => {
          ...
          if (!this.multiple) {
            ...
            if (option) {
              ...
              // 否则selected的watcher里面会触发$emit('input', undefined)，导致改变了外部传进来的值
              this.selected = {
                value: val
              };
              this.selectedLabel = '';
      ...
      selected(val, oldVal) {
        if (this.multiple) {
          ...
          if (this.filterable && this.visible) {
            // && this.visible 菜单没打开的情况下被修改不获取焦点
            this.query = '';
            this.hoverIndex = -1;
    ...
    methods: {
      ...
      handleOptionSelect(option) {
        ...
        this.$emit('option-select', this.selected, old);
      },

      handleSelectAllOptionSelect(selectAll) {
    ...
    created() {
      ...
      this.$on('handleSelectAllOptionClick', this.handleSelectAllOptionSelect);
