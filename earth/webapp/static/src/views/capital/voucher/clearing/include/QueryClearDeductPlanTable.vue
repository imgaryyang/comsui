    ...
    <div>
        <div class="row-layout-detail">
            <QueryTable
                ...
                :action="`clearingVoucher/${queryConds.auditJobUuid}/querySucDeductPlan?clearingVoucherUuid=${clearingVoucherUuid}`">
                ...
                <template slot="custom-table" scope="props">
                    <thead>
                        <tr><th :key="index" v-for="(title, index) in sucTableTitle" class="is-leaf"><div class="cell">{{title}}</div></th></tr>
                    </thead>
                    <tbody class="el-table--border">
                        <tr class="el-table__empty-block" v-if="props.data.length == 0"><td colspan="11">暂无数据</td></tr>
                        <template v-else v-for="(item, index) in props.data">
                            <tr>
                                ...
                                <td><div class="cell">
                                    <div class="el-table__expand-icon" :class="{'el-table__expand-icon--expanded':item.expand}" @click.prevet="expandDetail(index,item)">
    ...
    export default {
        ...
        data: function() {
            return {
                ...
                sucTableTitle: ['来源订单号', '单据类型', '对方账户', '交易金额', '借贷标记', '通道请求号', '状态变更时间', '执行状态', '清算状态', '操作', ''],
                detailList: {}
            }
        },
        computed:{
        ...
        watch: {
            auditJobIdList: function(current) {
