			...
			<div class="table-area">
				<el-table
					...
					<el-table-column
						inline-template
						label="期初">
						<span>{{ row.beginningInterest | formatMoney }}</span>
					</el-table-column>
					<el-table-column 
						inline-template
						label="本期新增">
						<span>{{ row.newInterest | formatMoney }}</span>
					</el-table-column>
					<el-table-column 
						prop="reduceInterest"
						inline-template
						label="本期减少">
						<span>{{ row.reduceInterest | formatMoney }}</span>
					</el-table-column>
					<el-table-column 
						prop="endingInterest"
						inline-template
						label="期末">
						<span>{{ row.endingInterest | formatMoney }}</span>
					</el-table-column>
				</el-table>
			</div>
		</div>
		<div class="operations">
			<div class="pull-left">
				<el-button 
					...
					size="small"
					v-if="ifElementGranted('export-interest')"
				    @click="download">
				    导出报表
				</el-button>
			</div>
...
<script>
	...
	export default {
		...
		data: function() {
			return {
				action: '/reportform/interest/query',
				pageConds: {
		...
		methods: {
			...
			getOptions: function() {
				return ajaxPromise({
					url: `/reportform/interest/options`
				}).then(data => {
			...
			download: function(){
				...
				var action = this.api + '/reportform/interest/exprot' + '?' + search;
