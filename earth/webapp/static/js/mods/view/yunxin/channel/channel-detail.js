define(function(require, exports, module) {
    ...
    var StrategyModel = Backbone.Model.extend({
        ...
        actions: {
            ...
            getBankChannelLimitPreview: root + '/paymentchannel/switch/strategy/bankChannelLimitPreview'
        },
        save: function() {
            var self = this;
        ...
        getBankChannelLimitPreview: function(callback) {
            var self = this;
            var opt = {
                dataType: 'json',
                url: self.actions.getBankChannelLimitPreview,
                data: _.pick(self.toJSON(), 'paymentChannelUuid', 'bankCode', 'accountSide')
            };
            opt.success = function(resp) {
                callback && callback(resp);
            };
    ...
    var NoChannelStrategy = StepOperationView.extend({
        templates: $('.no-channel .step-content-item .template').toArray().map(function(item) {
        ...
        initialize: function() {
            this.on('steped', function(index) {
                ...
                switch (index) {
                    case 1:
                        this.enteredStep1($content, tmplFn);
                        break;
                }
            });
            ...
            NoChannelStrategy.__super__.initialize.apply(this, arguments);
        },
    ...
    $.extend(NoChannelStrategy.prototype, mixins);
            ...
            this.on('steped', function(index) {
                ...
                switch (index) {
                    case 1:
                        ...
                        break;
                    case 2:
                        this.enteredStep2($content, tmplFn);
                        break;
                }
            });
    ...
    var IssuerStrategy = StepOperationView.extend({
        ...
        events: {
            ...
            'mouseout .showPopover': 'hidePopover'
        },
        showPopover: function(e) {
            ...
            this.timer = setTimeout(function() {
                ...
                self.model.set('paymentChannelUuid', $tar.data('paymentchanneluuid'));
                self.model.set('bankCode', $tar.data('bankcode'));
                self.model.getBankChannelLimitPreview(function(resp) {
                    $tar.popover({
                        content: function() {
                            if (resp.code == 0) {
                                var data = resp.data;
                                var paymentChannelName = _.isUndefined(data.paymentChannelName) ? '--' : data.paymentChannelName;
                                var bankName = _.isUndefined(data.bankName) ? '--' : data.bankName;
                                var transactionLimitPerTranscation = _.isUndefined(data.transactionLimitPerTranscation) ? '--' : (+data.transactionLimitPerTranscation).formatMoney(2, '');
                                var transcationLimitPerDay = _.isUndefined(data.transcationLimitPerDay) ? '--' : (+data.transcationLimitPerDay).formatMoney(2, '');
                                var transactionLimitPerMonth = _.isUndefined(data.transactionLimitPerMonth) ? '--' : (+data.transactionLimitPerMonth).formatMoney(2, '');

                                return '<div>' + paymentChannelName + '<br/>' + bankName +
                                    '<br/><span class="text-muted">单笔限额:</span>' + transactionLimitPerTranscation +
                                    '<br/><span class="text-muted">单日限额:</span>' + transcationLimitPerDay +
                                    '<br/><span class="text-muted">单月限额:</span>' + transactionLimitPerMonth + '<br/></div>';

                            } else {
                                return '<div>暂无相关信息</div>';
                            }
                        },
                        html: true
                    });
        ...
        initialize: function() {
            this.on('step', function(index) {
                switch (index) {
                    case 2:
                        this.beforeLeaveStep2();
                        break;
                    case 3:
                        this.beforeLeaveStep3();
                        break;
                }
            });
            ...
            this.on('steped', function(index) {
                ...
                switch (index) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        this.enteredStep3($content, tmplFn);
                        break;
                    case 4:
                        this.enteredStep4($content, tmplFn);
                        break;
                }
            });
        ...
        enteredStep3: function($el, tmplFn) {
            ...
            model.getAllBank(function(resp) {
                ...
                self.radioController = new RadioCheckboxControl($el.find('.data-list tbody'), {
                    groupSelector: 'tr'
                });

                if (!model.bankList || !model.bankList.length) {
                    self.disableNextStop();
                }
            });
        },
    ...
    var SetChannelStrategyView = FormDialogView.extend({
        ...
        initialize: function() {
            ...
            this.noChannelStrategy = new NoChannelStrategy({
                model: this.model,
                el: this.$('.no-channel').get(0)
            });
            ...
            this.listenTo(this.noChannelStrategy, 'step:done', function() {
                this.model.save();
                this.hide();
            });

            this.listenTo(this.singleStrategy, 'step:cancel', function() {
                this.hide();
            });
            this.listenTo(this.issuerStrategy, 'step:cancel', function() {
                this.hide();
            });
            this.listenTo(this.noChannelStrategy, 'step:cancel', function() {
                this.hide();
            });

            this.singleStrategy.show();
            this.issuerStrategy.hide();
            this.noChannelStrategy.hide();
    ...
    var ChannelDetailView = Backbone.View.extend({
        ...
        onClickSetChannelStrategy: function(e) {
            ...
            var view = new SetChannelStrategyView({
                model: model
            });
            view.show();
        },
        onClickPreviewPriority: function(e) {
            ...
            var view = new PreviewPriorityView({
                model: model
            });
            view.show();
        },
