define(function(require, exports, module) {
    ...
    var PaymentView = FieldsetView.extend({
        events: {
            ...
            'click .upload-files .delete-files': 'onClickDeleteFiles'
        },
        actions: {
            upload: function() {
                var search = [
            ...
            deleteFile: global_config.root + '/paymentchannel/file/delete'
        },
        initialize: function() {
        ...
        onClickDeleteFiles: function(e) {
            ...
            var opt = {
                type: 'post',
                dataType: 'json',
                url: self.actions.deleteFile,
                data: {
                    paymentInstitutionName: self.paymentInstitutionOrdinal,
                    outlierChannelName: self.outlierChannelName,
                    accountSide: self.accountSide
                }
            };
            opt.success = function(resp) {
                if (resp.code == 0) {
            ...
            FileUploadPlugin.globalUploadFile(this.$('.file-input'), {
                ...
                createItemDom: function(data, sourceFile) {
                    var file = '<li class="item" data-filename="' + data.fileName + '" data-filekey="' + data.fileKey + '">' +
                        ...
                        '<a href="#" class="delete-files" style="margin-left:10px">删除</a>' +
                        '</li>';
                    return file;
                }
            });
        ...
        defineValidator: function() {
            this.validator = this.$('form').validate({
                ...
                rules: {
                    ...
                    trasncationLimitPerTransactionMin: {
                        nonNegativeNumber: true,
                        minRelatedElement: $.proxy(function() {
                            return this.$('[name=trasncationLimitPerTransactionMax]');
                        }, this)
                    },
                    trasncationLimitPerTransactionMax: {
                        nonNegativeNumber: true,
                        maxRelatedElement: $.proxy(function() {
                            return this.$('[name=trasncationLimitPerTransactionMin]');
                        }, this)
                    },
                ...
                messages: {
                    ...
                    trasncationLimitPerTransactionMin: {
                        minRelatedElement: '最高单笔限额应大于最低单笔限额'
                    },
                    trasncationLimitPerTransactionMax: {
                        maxRelatedElement: '最高单笔限额应大于最低单笔限额'
                    }
                },
    ...
    var QuotaPreview = FormDialogView.extend({
        ...
        // },
        limitSheetListTemplate: _.template($('#limitSheetListTmpl').html()),
        versionLogListTemplate: null,
        initialize: function() {
            ...
            var self = this;
            var LimitSheetListPagination1 = Pagination.extend({
                template: this.limitSheetListTemplate,
                collectParams: function() {
                    return self.model.toJSON();
                }
            });
            var VersionLogListPagination1 = Pagination.extend({
                template: this.versionLogListTemplate,
                collectParams: function() {
                    return self.model.toJSON();
                }
            });

            var $segmentContentItems = this.$('.segment-content-item');

            var pagination1 = new LimitSheetListPagination1({
                el: $segmentContentItems.get(0)
            });
            var pagination2 = new VersionLogListPagination1({
                el: $segmentContentItems.get(1)
            });
    ...
    var ConfigEditView = BaseFormView.extend({
        ...
        initSubView: function() {
            ...
            this.listenTo(this.confirm, 'goahead', function() {
                ...
                location.href = global_config.root + '/v#/financial/channel/switch/detail/' + this.model.get('relatedFinancialContractUuid');
            }).listenTo(this.confirm, 'closedialog', function() {
                location.href = global_config.root + '/v#/financial/channel/configure/list';
            });
        },
        validate: function() {
