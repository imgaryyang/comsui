define(function(require, exports, module) {
    ...
    var ExportPreviewDialogView = require('./exportPreviewDialogView');
    ...
    var TableContentView = Backbone.View.extend({
        ...
        events: {
            ...
            'change .lookup-params': function(e) {
                ...
                if ($(e.target).hasClass('select-key-word')) return;
                this._queryByParams();
            },
        ...
        initialize: function() {
            ...
            $('body').on('click', function(event) {
                ...
                if (!target.hasClass('pagecontrol-show-popover') && $('.popover').length > 0 && !target.hasClass('popover') //弹窗内部点击不关闭
                    && target.parent('.popover-content').length === 0 && target.parent('.popover-title').length === 0 && target.parent('.popover').length === 0 && target.data('toggle') !== 'popover') {
                    //弹窗触发列不关闭，否则显示后隐藏
                    $('.popover').popover('destroy');
                }
            });
        },
        showStatisticalAmountPopover: function(e) {
            e.preventDefault();

            var el = $(e.target);
            this.destroyOtherPopover();
            var val = this.$('.selectpicker').first().selectpicker('val');
            var content;
            var queryAmount = false;
            el.popover();

            if (val == null) {
                content = '还未选择信托项目';
            } else if (val.length > 1) {
                content = '项目数量>1超出统计范围';
            } else {
                content = '金额统计中...';
                queryAmount = true;
            }
            ...
            this.trigger('show-popover:amountStatistics', queryAmount);
        },
        destroyOtherPopover: function() {
        ...
        initPageControl: function() {
            ...
            this.pageControl.on('request', function() {
            ...
            this.pageControl.on('pagecontrol:bad', function(resp) {
                var htm = '<tr class="nomore"><td style="text-align: center;" colspan="100">' + resp.message + '</td></tr>';
                tableListEl.find('tbody').html(htm);
            });
        },
        ...
        collectParams: function() {
            ...
            resultParams = this.filterKeyWord(resultParams);

            return resultParams;
        },
        filterKeyWord: function(resultParams) {
            var key = resultParams.selectKeyWord;
            var value = resultParams.keyWords;

            if (value == undefined) {
                return _.omit(resultParams, 'selectKeyWord');
            }
            resultParams[key] = value;
            return _.omit(resultParams, 'selectKeyWord', 'keyWords');
        },
        ...
        onClickExport: function(e) {
            ...
            var previewAction = $target.data('preview-action');
            var previewTemplate = $target.data('preview-template');

            if (previewAction && previewTemplate && action) {
                var _action = path.format({
                    path: action,
                    query: this.collectParams()
                });

                var _previewAction = path.format({
                    path: previewAction,
                    query: this.collectParams()
                });

                var view = new ExportPreviewDialogView({
                    itemTemplate: _.template($(previewTemplate).html()),
                    trigger: e.target,
                    downloadAction: _action,
                    queryAction: _previewAction
                });
