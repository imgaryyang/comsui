...
<body>
		...
		<div class="content">
			<div class="static-info-wrapper">
				...
				<div class="lookup-params">
					...
					<span class="item">
						<button id="doDeductWriteOff" class="btn btn-primary">扣款核销</button>
					</span>
					<span class="item">
					...
					<span class="item">
						<button id="doRepurchaseTask" class="btn btn-primary">执行回购</button>
					</span>
					<span class="item">
						<button id="doAutoRecoverTask" class="btn btn-primary">自动核销</button>
					</span>
				</div>
				<div>Unionpay 相关：</div>
				<div class="lookup-params">
				...
				<div>接口数据同步：</div>
				<div class="lookup-params">
				<form id="myFormDataSync" action="" method="GET">
				    <span class="item">
				         <textarea   name="singleContractNo"   style="height:100px; width:48px" class="form-control">
				         </textarea>
	...
	<script type="text/javascript">
		$(function() {
			...
			$("#doTaskDataSync").click(function(e) {
				var data = $("#myFormDataSync").serialize();
				e.preventDefault();
				e.stopPropagation();
				window.open("${ctx}/manual-tasks/data-sync?" + data);
		...
		$(function() {
			...
			//扣款核销
			ajaxGet("#doDeductWriteOff",
					"/manual-tasks/deduct-write-off");
			//还款提醒短信
			...
			//执行回购
			ajaxGet("#doRepurchaseTask","/manual-tasks/exec-repurchasing");
			//执行自动核销
			ajaxGet("#doAutoRecoverTask","/manual-tasks/exec-auto-recover");
		});
	</script>
</body>
</html>