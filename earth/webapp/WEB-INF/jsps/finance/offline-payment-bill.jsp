				...
				<div class="lookup-params">
					<div class="inner clearfix">
						<div class="pull-left">
							...
							<span class="item">
								<select name="financialContractUuids" id="financialContractUuids" class="form-control real-value selectpicker" multiple data-actions-box="true" data-title="请选择信托合同项目">
					...
					<table class="data-list" data-action="./query" data-autoload="true">
						<thead>
							<tr>
								...
								<th>关联状态</th>
								<th>
									<a data-paramname="tradeTime" class="sort none">
										入账时间<i class="icon"></i>
									</a>
								</th>
								<th>
									<a data-paramname="statusModifiedTime" class="sort none">
										发生时间<i class="icon"></i>
									</a>
								</th>
								<th>支付状态</th>
							...
							{% var OfflineBillStatus = { UNPAID:'失败',PAID:'成功' }; %}
							    {% _.each(list, function(item, index) { %}
							        <tr class="record-item">
							            ...
							            <td>
							            	{%	if(item.offlineBill.offlineBillStatus == 'UNPAID'){ %}
													<span class="color-danger">失败</span>
											{%  }else{  %}
							            			{%= OfflineBillStatus[item.offlineBill.offlineBillStatus]%}
											{%  }  %}
							            </td>
