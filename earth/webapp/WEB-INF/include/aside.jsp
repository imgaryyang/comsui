		                            ...
		                            <sec:authorize access="hasResource('submenus', '${submenu.id}')"> 
		                                <li class="menu-item ${not empty submenu.submenus ? 'has-submenu' : '' }  ${submenu.id == activeSubmenu ? 'z-active' : ''}">
		                                	<c:choose>
			                                		...
			                                		<ul class="sub-menu">
			                                			<c:forEach var="secondSubMenu" items="${submenu.submenus }">
			                                				<sec:authorize access="hasResource('submenus', '${secondSubMenu.id}')">
				                                				<li class="sub-menu-item ${secondSubMenu.id ==activeSubmenu ? 'z-active' : ''}">
				                                					<a class="real-menu-link" href="${ctx}/${secondSubMenu.url}" title="${secondSubMenu.description}">${secondSubMenu.name}</a>
				                                				</li>
			                                				</sec:authorize>
			                                			</c:forEach>
			                                		</ul>
		                                		...
		                                		<c:otherwise>
		                                			<a class="menu-link real-menu-link" href="${ctx}/${submenu.url }" title="${submenu.description}">${submenu.name}</a>
		                                		</c:otherwise>
		                                	</c:choose>
		                                </li>
		                            </sec:authorize>  
