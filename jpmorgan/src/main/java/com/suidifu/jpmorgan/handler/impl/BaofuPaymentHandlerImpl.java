...
@Component("baofuPaymentHandler")
public class BaofuPaymentHandlerImpl implements PaymentHandler {

	@Autowired
	@Qualifier(value = "baofuHandler")
	private ThirdPartyPayHandler baofuHandler;
	...
	public CreditResult executeSinglePay(PaymentOrder paymentOrder,
		...
		try {
			...
			CreditResult creditResult = baofuHandler.singleDebit(creditModel,
					context.getWorkingParameters());
			return creditResult;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public QueryCreditResult executeQueryPaymentStatus(
		...
		try {
			...
			QueryCreditResult queryCreditResult = baofuHandler.queryDebit(
