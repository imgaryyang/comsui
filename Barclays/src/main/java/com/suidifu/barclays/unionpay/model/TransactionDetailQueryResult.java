package com.suidifu.barclays.unionpay.model;
...
public class TransactionDetailQueryResult {
	...
	public static TransactionDetailQueryResult initialization(String rspPacket) {
		...
		if (CollectionUtils.isEmpty(queryResult.getDetailNodes())) {
			queryResult.setDetailNodes(Collections.EMPTY_LIST);
		}
		return queryResult;
	}
	
	public static GZUnionPayRtnInfo parseInfo(String rspPacket){
		try {
			Document document = DocumentHelper.parseText(rspPacket);
