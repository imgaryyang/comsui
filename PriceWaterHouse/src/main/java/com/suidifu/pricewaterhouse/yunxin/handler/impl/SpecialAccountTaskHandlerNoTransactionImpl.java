 ...
 *
 */
@Component("SpecialAccountTaskHandlerNoTransaction")
public class SpecialAccountTaskHandlerNoTransactionImpl extends TaskDeploy implements SpecialAccountTaskHandlerNoTransaction,CriticalSecionProjector {
...
//				if(context.getJob().getFstStageExcutingStatus() != ExecutingStatus.DONE){
...
//				}
				...
				logger.error("handle_special_account_book_accounts occour error with exception with jobIdentity(batchuuid)["+jobIdentity+"],eception stack trace :"+ExceptionUtils.getStackTrace(e));
	...
	private void registSubrogationJob(){
		//List<batchUuid>
		List<SpecialAccountPrepareParamsModel> specialAccountPrepareParamsModelList = specialAccountNoTransaction.prepareForClearingExecLog();
		...
		for(SpecialAccountPrepareParamsModel specialAccountPrepareParamsModel : specialAccountPrepareParamsModelList){
				...
				JobType jobTypeForSubrogation = JobType.SPECIAL_ACCOUNT_RECORD;
				  
			try{
	         
				Job job = jobService.getAllJobBy(specialAccountPrepareParamsModel.getBatchUuid(), specialAccountPrepareParamsModel.getFinancialContratUuid(), StringUtils.EMPTY, jobTypeForSubrogation);
	          ...
	          if (null == job) {
	               ...
	               Job jobForSubrogation = jobHandler.registeJob(specialAccountPrepareParamsModel.getBatchUuid(), specialAccountPrepareParamsModel.getFinancialContratUuid(), StringUtils.EMPTY, jobTypeForSubrogation);
	               String beanNameForSubrogation = "dstJobClearingAssetReconciliation";
	...
	private List<SpecialAccountTaskHandlerModel> collectNeedToDoJobTarget(){
		...
		List<Job> createdOrProcessingJobList = jobService.loadAllCreateOrProcessingJobBy(JobType.SPECIAL_ACCOUNT_RECORD);
	...
	private HashMap<String,JobContext<SpecialAccountTaskHandlerModel>> prepareJobContext(List<SpecialAccountTaskHandlerModel> specialAccountTaskHandlerModelList){
		...
		for(SpecialAccountTaskHandlerModel specialAccountTaskHandlerModel : specialAccountTaskHandlerModelList){
			...
			List<ClearingExecLog> clearingExecLogOfNeedClearList = clearingExecLogService.queryNeedToRecordOfNeedClear(JournalVoucherType.JournalVoucherTypeUsedSpecialAccount(), batchUuid);
			
			FinancialContract financialContract = financialContractService.getFinancialContractBy(clearingExecLogOfNeedClearList.get(0).getFinancialContractUuid());
			JobContext<SpecialAccountTaskHandlerModel> jobContext=new JobContext<>();
			
			Job job = jobService.getJobBy(batchUuid, JobType.SPECIAL_ACCOUNT_RECORD);
			
			Map<String, String> syncMarks = new HashMap<>();
			List<AssetClearingReconParameters> assetClearingReconParametersList = new ArrayList<>();
			
			for(ClearingExecLog clearingExecLog : clearingExecLogOfNeedClearList){
				...
				AssetClearingReconParameters assetClearingReconParameters = new AssetClearingReconParameters(clearingExecLog.getUuid(),
						clearingExecLog.getFinancialContractUuid(),financialContract.getLedgerBookNo(),batchUuid);
