...
public class PrepaymentHandlerImpl implements PrepaymentHandler {
    ...
    @Override
    public void processingOnePrepaymentPlan(String contractUuid,
                                            int priority, Long prepaymentApplicationId) {
    ...
    private boolean prepaymentPlanRollBack(PrepaymentApplication application, AssetSet prepaymentPlan) {
        ...
        if (CollectionUtils.isNotEmpty(failDeductApplicationList) || DateUtils.getToday().after(prepaymentPlan.getAssetRecycleDate())) {
            String errMsg = invalidPrepaymentAndUndoFrozenBePreRepaymentPlan(application, prepaymentPlan);
     ...
     */
    @Override
    public String invalidPrepaymentAndUndoFrozenBePreRepaymentPlan(PrepaymentApplication application, AssetSet prepaymentPlan) {
        try {
            ...
            //提前还款不存在部分还款
            //作废提前还款
            log.info("#将提前还款作废并将贷款合同回滚至之前的版本， 提前还款计划uuid = ({})",
                    prepaymentPlan.getAssetUuid());
            repaymentPlanService.wirteOffRepaymentPlan(Collections.singletonList(prepaymentPlan),
                    WriteOffReason.PRE_REPAYEMNT_PLAN,
            ...
            if (ledgerBookV2Handler.checkLedgerbookVersionByledgerBookNo
                ...
                log.info("begin to PrepaymentHandlerImpl" +
                        "#invalidPrepaymentAndUndoFrozenBePreRepaymentPlan make#is book_loan_assets_v2_pre !!!");
            ...
            log.error("#processingPrepaymentPlan occur error, prepayment plan uuid = [{}]",
                    prepaymentPlan.getAssetUuid());
            return "提前还款作废失败！";
        }
    }
    ...
    private void undoFrozenPrepaymentPlan(AssetSet prepaymentPlan) {
        ...
        Integer count = repaymentPlanService.
                count_open_and_unclear_asset_list_before_prepayment_plan(prepaymentPlan.getContractUuid(), prepaymentPlan.getCurrentPeriod());
    ...
    private boolean isExistPartDeduct(AssetSet prepaymentPlan) {
        ...
        BigDecimal ledgerBookAmount = ledgerBookStatHandler.unrecovered_asset_snapshot(ledgerBookNo, prepaymentPlan.getAssetUuid(), prepaymentPlan.getCustomerUuid(), true);
