...
public class SourceDocumentDetailServiceImpl extends
  ...
  public List<ContractSourceDocumentDetailMapper> getContractUuidSourceDocumentDetailUuidMapper(
    ...
    String sql = "SELECT sdd.uuid AS sourceDocumentDetailUuid,"
        ...
        + " FROM source_document_detail sdd LEFT JOIN contract c ON sdd.contract_unique_id = c.unique_id  WHERE sdd.uuid IN ('" + inListStr + "') ";

    List<ContractSourceDocumentDetailMapper> result = this.genericDaoSupport.queryForList(sql, ContractSourceDocumentDetailMapper.class);
