...
public class ReconciliationForRepurchaseDocumentHandler extends ReconciliationForSourceDocument {
    ...
    public ReconciliationContext preAccountReconciliation(
        ...
        context.resolveJournalVoucher(AccountSide.DEBIT, CounterAccountType.VirtualAccount, context.getRepurchaseDoc().getUuid());
    ...
    public void postAccountReconciliation(ReconciliationContext context) {
        if (BigDecimal.ZERO.compareTo(ledgerBookStatHandler.get_repurchase_amount(context.getLedgerBookNo(),
                context.getContract().getUuid())) == 0) {
