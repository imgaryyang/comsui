...
public class LedgerBookClearingVoucherHandlerImpl implements LedgerBookClearingVoucherHandler {
    ...
    public void clearingVoucherWriteOff(String ledgerBookNo,
        ...
        for (SourceDocumentDetail sourceDocumentDetail : detailList) {
            ...
            if (journalVoucher != null) {
                ...
                List<LedgerItem> ledgerItemList = ledgerItemService.get_jv_asset_accountName_related_ledgers(ledgerBookNo,
                        asset.getAssetUuid(),
                        journalVoucher.getJournalVoucherUuid(),
                        Collections.singletonList(ChartOfAccount.FST_BANK_SAVING));
                ...
                if (!CollectionUtils.isEmpty(ledgerItemList)) {
                    ...
                    for (String accountName : accountNameMutilItemsMap.keySet()) {
                        BigDecimal amountInDb = accountNameMutilItemsMap.get(accountName).
                                stream().map(LedgerItem::getDebitSubCreditBalance).
                    ...
                    Map<String, List<LedgerItem>> ledgersMap = ledgerItemService.carry_over_by_account_balance_table2(book, assetCategory,
                    ...
                    List<LedgerItem> savingLedgers = new ArrayList<>();
    ...
    private List<LedgerItem> bankSavingLedgersWashing(DepositeAccountInfo depositeAccountInfo, List<LedgerItem> bankSavingLedgers) {
        List<LedgerItem> washingResult = new ArrayList<>(bankSavingLedgers);
        TAccountInfo accountInfo = null;
        if (depositeAccountInfo != null) {
            String toBankAccountName = depositeAccountInfo.getDeposite_account_name();
            accountInfo = ChartOfAccount.EntryBook().get(toBankAccountName);
        }

        if (accountInfo == null) return washingResult;
        if (!accountInfo.getFirstLevelAccount().getAccountName().
                equals(ChartOfAccount.FST_BANK_SAVING))
