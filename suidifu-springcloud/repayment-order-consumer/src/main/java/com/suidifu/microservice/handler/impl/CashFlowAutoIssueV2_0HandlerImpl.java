...
public class CashFlowAutoIssueV2_0HandlerImpl implements CashFlowAutoIssueV2_0Handler {
    ...
    @Override
    public void recoverAssetsRepaymentOrderDeduct(String sourceDocumentUuid) {
        logger.info("recoverAssetsRepaymentOrderDeduct start, sourceDocumentUuid[" + sourceDocumentUuid + "].");
        ...
        if (deductApplication == null || !deductApplication.isPartSucOrSus()) {
            logger.info("recoverAssetsRepaymentOrderDeduct start, deductApplication is null or not success, sourceDocumentUuid[" + sourceDocumentUuid + "],deductApplicationUuid[" + (deductApplication == null ? "" : deductApplication.getDeductApplicationUuid()) + "].");
            return;
        }

        List<String> sourceDocumentDetailUuids = sourceDocumentDetailService.getValidDeductSourceDocumentDetailUuidsBy(sourceDocumentUuid);
        for (String sourceDocumentDetailUuid : sourceDocumentDetailUuids) {
            ...
            logger.info("recoverAssetsRepaymentOrderDeduct by each item start, itemUuid[" + sourceDocumentDetail.getSecondNo() + "], sourceDocumentUuid[" + sourceDocumentUuid + "].");
            accountReconciliation.accountReconciliation(inputParams);
            logger.info("recoverAssetsRepaymentOrderDeduct by each item end, itemUuid[" + sourceDocumentDetail.getSecondNo() + "], sourceDocumentUuid[" + sourceDocumentUuid + "].");
        }
        ...
        logger.info("recoverAssetsRepaymentOrderDeduct end, sourceDocumentUuid[" + sourceDocumentUuid + "].");
    }

    @Override
    public void recoverReceivableInAdvance(String orderUuid, String paymentOrderUuid) {
