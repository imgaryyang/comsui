...
import com.suidifu.microservice.handler.ReconciliationRepayment;
...
public abstract class ReconciliationRepaymentBase implements ReconciliationRepayment {
    ...
    void extractReconciliationContextFromAssetSet(ReconciliationRepaymentContext context, AssetSet assetSet) {
        ...
        log.debug("#abcdefgh#preAccountReconciliation#extractReconciliationContextFromAssetSet#extract asset,contract use [" + (end - start) + "]ms");
    ...
    private void extractReconciliationContextFrom(ReconciliationRepaymentContext context, AssetSet assetSet, Contract contract) {
        ...
        if (assetSet != null) {
            ...
            context.setBorrowerDepositAccountForLedgerBook(borrowerDepositeAccountForLedegerBook);
        }

        long end_3 = System.currentTimeMillis();
        log.debug("#abcdefgh#preAccountReconciliation#extractReconciliationContextFromAssetSet#extract DepositeAccountInfo use [" + (end_3 - end_2) + "]ms");
        ...
        log.debug("#abcdefgh#preAccountReconciliation#extractReconciliationContextFromAssetSet#extract rest use [" + (end_4 - end_3) + "]ms");

        extractRelatedBalnaceFromLedgerBook(context);
    }

    private void extractRelatedBalnaceFromLedgerBook(ReconciliationRepaymentContext context) {
        ...
        if (context.getReconciliationType().isRecoveredByVirtualAccountSelf()) {
            BigDecimal borrowerCustomerBalance = ledgerBookVirtualAccountHandler.getBalanceOfCustomer(ledgerBookNo, borrowerCustomer.getCustomerUuid());
            context.setBorrowerCustomerBalance(borrowerCustomerBalance);
        } else {
            BigDecimal financialAppBalance = ledgerBookVirtualAccountHandler.getBalanceOfCustomer(ledgerBookNo, companyCustomer.getCustomerUuid());
    ...
    public void processIfPrepaymentPlan(ReconciliationRepaymentContext context) {
        //提前还款解冻
    ...
    public boolean accountReconciliation(Map<String, Object> params) {
        try {
            ...
            log.info("#accountReconciliation# MisMatchedConditionException occur exception,error message :" + e);
            return false;
        } catch (AlreadyProcessedException e) {
            log.info("#accountReconciliation# AlreadyProcessedException occur exception,error message :" + e);
            return true;
        }
    }
    ...
    public void refreshVirtualAccount(ReconciliationRepaymentContext context) {
        ...
        if (context.getReconciliationType().isRecoveredByVirtualAccountSelf()) {
            VirtualAccount refreshedVirtualAccount = virtualAccountHandler.refreshVirtualAccountBalance(financialContract.getLedgerBookNo(), context.getRemittanceVirtualAccount().getOwnerUuid(), financialContract.getFinancialContractUuid(), context.getVirtualAccountVersionLock());
    ...
    public void refreshAsset(ReconciliationRepaymentContext context) throws GiottoException {
        ...
        updateAssetStatusLedgerBookHandler.updateAssetsFromLedgerBook(context.getLedgerBookNo(), Collections.singletonList(context.getAssetSet().getAssetUuid()), context.getActualRecycleTime(), repaymentType, context.getAssetSet().getExecutingStatus());
